...
class NoMASS(object):
    ...
    def configuration(self, x):
        ...
        print("configuration. rl + self.simulationFile", rl + self.simulationFile)
        self.root = root

    def copyToResultsLocation(self, x):
        rl = self.resultsLocation
        print("copyToResultsLocation.rl",rl)
        if not os.path.exists(rl):
            os.makedirs(rl)
        print("self.outFiles", self.outFiles)
        if self.outFiles:
            outfileStr = "NoMASS-" + str(x).zfill(5) + ".out"
            copyfile(self.runLoc(x)+self.outFile, rl+outfileStr)
            print("From", self.runLoc(x)+self.outFile, "To", rl+outfileStr)
    ...
    def copyToRunLocation(self, x):
        rl = self.runLoc(x)
        print("copyToRunLocation.x.rl", x, rl)
        if not os.path.exists(rl):
            os.makedirs(rl)
        print("From", self.locationOfNoMASS+self.NoMASSstr, " To", rl+self.NoMASSstr)
        copyfile(self.locationOfNoMASS+self.NoMASSstr, rl+self.NoMASSstr)
        con = self.configurationDirectory
        print("con", con)
        print("simulationFile", con + self.simulationFile, rl + self.simulationFile)
        copyfile(con + self.simulationFile, rl + self.simulationFile)
        print("activityFile", con + self.activityFile, rl + self.activityFile)
        copyfile(con + self.activityFile, rl + self.activityFile)
        print("learningXMLFile", con + self.learningXMLFile, rl + self.learningXMLFile)
        copyfile(con + self.learningXMLFile, rl + self.learningXMLFile)
        print("largeApplianceFile", con + self.largeApplianceFile, rl + self.largeApplianceFile)
        copyfile(con + self.largeApplianceFile, rl + self.largeApplianceFile)
        print("HeatingPowerFile", con + self.HeatingPowerFile, rl + self.HeatingPowerFile)
        copyfile(con + self.HeatingPowerFile, rl + self.HeatingPowerFile)
        print("PVFile", con + self.PVFile, rl + self.PVFile)
        copyfile(con + self.PVFile, rl + self.PVFile)
        print("self.appFiles", self.appFiles)
    ...
    def makeExecutable(self, x):
        ...
        print("makeExecutable", "rl", rl, "nomassexe", nomassexe)
    ...
    def createPandasFiles(self, x, filename):
		print("createPandasFiles", self.pandasFiles)	
		if self.pandasFiles:
