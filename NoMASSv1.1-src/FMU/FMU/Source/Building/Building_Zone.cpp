...
//   Update -- daps -- 3.1.2018 eplusVersion verification for naming of variable "DaylightingReferencePoint1Illuminance"
...
void Building_Zone::setup(const ConfigStructZone & zoneStruct)
	...
	/*if (active)
		LOG << "   zone active = TRUE" << "\n";
	else
		LOG << "   zone active = FALSE" << "\n";*/
    ...
    if (active)
		...
		//LOG << "   isZoneGroundFloor:" << Configuration::isZoneGroundFloor(&name) << "\n";
        ...
        if(Configuration::info.eplusVersion < 8.6)
            {
			//LOG << " Configuration::info.eplusVersion < 8.6: " << name + "DaylightingReferencePoint1Illuminance" << "\n";
            variableNameDaylighting =
                DataStore::addVariable(name + "DaylightingReferencePoint1Illuminance");
            }
        else
            {
			//LOG << " Configuration::info.eplusVersion >= 8.6: " << name + "_DaylCtrl" + "DaylightingReferencePoint1Illuminance" << "\n";
            variableNameDaylighting =
                DataStore::addVariable(name + DAYLCTRLVARNAME + "DaylightingReferencePoint1Illuminance");
            }
        variableNameZoneAirHeating =
            DataStore::addVariable(name + "ZoneAirSystemSensibleHeatingRate");
        }
	//LOG << "         end of zone setting up" << "\n";
    }
