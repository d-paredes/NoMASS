...
void Building::setup(const ConfigStructBuilding &buildingInput)
	...
	//LOG << "     " << buildingID << " (" << id << ") " << name << "\n";
    for (std::pair<std::string, ConfigStructZone> z : buildingInput.zones)
        {
		//LOG << "          +Zone + : name: " << z.second.name << " active: " << z.second.active << " setting up..." << "\n";
	...
	//LOG << "agents done!!!" << "\n";
	//LOG << "appliances.setup..." << "\n";
    appliances.setup(buildingInput);
	//LOG << "appliances.setup done!!!" << "\n";
...
void Building::preprocess()
    {
	//LOG << "Building::preprocess(), details calls the building appliances preprocess ..." << "\n";
