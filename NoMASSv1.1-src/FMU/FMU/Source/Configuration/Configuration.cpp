...
//   Update -- daps -- 3.1.2017 Variable eplusVersion loaded from SimulationConfig.xml
        ...
        if (nodeNameIs(cnode, "name"))
            ...
            while (znode)
                ...
                if (nodeNameIs(znode, "name"))
                    ...
                    //LOG << "   Number of windows: [" << zone.second.windowCount << "]" << "\n";
                    }
...
void Configuration::parseOccupants(rapidxml::xml_node<> *node,
    ...
    while (cnode)
        ...
        if (nodeNameIs(cnode, "agent"))
            ...
            while (anode)
                ...
                setValFromNodeIfName(&agent.windowId, value, name, "windowid"); // setValFromNodeIfName(&agent.windowId, value, name, "window");
                setValFromNodeIfName(&agent.shadeId, value, name, "shadeid"); // setValFromNodeIfName(&agent.shadeId, value, name, "shade");
                setValFromNodeIfName(&agent.edtry, value, name, "edtry");
                setValFromNodeIfName(&agent.age, value, name, "agegroup"); // setValFromNodeIfName(&agent.age, value, name, "age");
                setValFromNodeIfName(&agent.computer, value, name, "owncomputer"); // setValFromNodeIfName(&agent.computer, value, name, "computer");
                setValFromNodeIfName(&agent.civstat, value, name, "civstategroup"); // setValFromNodeIfName(&agent.civstat, value, name, "civstat");
                setValFromNodeIfName(&agent.unemp, value, name, "isunemployed"); // setValFromNodeIfName(&agent.unemp, value, name, "unemp");
                setValFromNodeIfName(&agent.retired, value, name, "isretired"); // setValFromNodeIfName(&agent.retired, value, name, "retired");
                setValFromNodeIfName(&agent.sex, value, name, "sex"); // setValFromNodeIfName(&agent.retired, value, name, "sex");
                setValFromNodeIfName(&agent.famstat, value, name, "famstategroup"); // setValFromNodeIfName(&agent.famstat, value, name, "famstat");
            ...
            else if (!Configuration::info.presence
    ...
    //LOG << "info.presence [" << info.presence << "]\n";
    }
...
void Configuration::parseAgentHeatGains(rapidxml::xml_node<> *node)
    {
    for (rapidxml::xml_attribute<> *attr=node->first_attribute(); attr; attr=attr->next_attribute())
        {
        if (attributeNameIs(attr, "enabled"))
            {
            Configuration::info.agentHeatGains = std::stoi(attr->value());
            break;
            }
        }
    }

void Configuration::parseLights(rapidxml::xml_node<> *node)
    {
    for (rapidxml::xml_attribute<> *attr=node->first_attribute(); attr; attr=attr->next_attribute())
        {
        if (attributeNameIs(attr, "enabled"))
            {
            Configuration::info.lights = std::stoi(attr->value());
            break;
            }
        }
    }

void Configuration::parseHeating(rapidxml::xml_node<> *node)
    {
    for (rapidxml::xml_attribute<> *attr=node->first_attribute(); attr; attr=attr->next_attribute())
        {
        if (attributeNameIs(attr, "enabled"))
            {
            Configuration::info.heating = std::stoi(attr->value());
            break;
            }
        }
    }
...
void Configuration::parseWindows(rapidxml::xml_node<> *node)
    {
    //LOG << "Configuration::parseWindows()" << "\n";
    windows.clear();
    for (rapidxml::xml_attribute<> *attr=node->first_attribute(); attr; attr=attr->next_attribute())
        {
        if (attributeNameIs(attr, "enabled"))
            {
            Configuration::info.windows = std::stoi(attr->value());
            break;
            }
        }
    rapidxml::xml_node<> *cnode = node->first_node();
...
//            }
            ...
            windows.insert(ws);
            //LOG << "            Number of Windows:" << windows.size() << "\n";
            }
        cnode = cnode->next_sibling();
        }

    //LOG << "Configuration::parseWindows, " << "Configuration::info.windows:[" << Configuration::info.windows << "]" << "\n";
    }
...
void Configuration::parseShades(rapidxml::xml_node<> *node)
    ...
    for (rapidxml::xml_attribute<> *attr=node->first_attribute(); attr; attr=attr->next_attribute())
        ...
        if (attributeNameIs(attr, "enabled"))
            {
            Configuration::info.shading = std::stoi(attr->value());
            break;
            }
        }
    rapidxml::xml_node<> *cnode = node->first_node();
...
void Configuration::parseConfiguration(const std::string & filename)
    ...
    while (node)
        ...
        setValFromNodeIfName(&info.eplusVersion, value, name, "eplusversion");
    ...
    // print configuration
        {
        std::map<std::string, ConfigStructZone>::iterator itZones;

        LOG << "+--------------------------------------------------" "\n";
        LOG << "\n";

        LOG << "fmuNoMASS: " << FULLVERSION_STRING << " [" << YEAR << "." << MONTH << "." << DATE << "]" "\n";

        LOG << "\n";
        LOG << "eplusVersion: " << info.eplusVersion << "\n";
        LOG << "TimeStepsPerHour: " << info.timeStepsPerHour << "\n";
        LOG << "\n";

        LOG << "Number of zones (Buildings[0]): [" << Configuration::buildings[0].zones.size() << "]" << "\n";
        for(itZones=Configuration::buildings[0].zones.begin(); itZones!=Configuration::buildings[0].zones.end(); itZones++)
            {
            LOG << "   " << itZones->first << "\n";
            }
        LOG << "\n";
        ...
        LOG << "NoMASS models: \n";
        LOG << " - Presence: " << (Configuration::info.presence ? "True" : "False") << "\n";
        LOG << " - Windows: " << (Configuration::info.windows ? "True" : "False") << "\n";
        LOG << " - Shading: " << (Configuration::info.shading ? "True" : "False") << "\n";
        LOG << " - Lights: " << (Configuration::info.lights ? "True" : "False") << "\n";
        LOG << " - Heating: " << (Configuration::info.heating ? "True" : "False") << "\n";
        LOG << " - Agent Heat Gains: " << (Configuration::info.agentHeatGains ? "True" : "False") << "\n";
        LOG << "\n";

        LOG << " * Windows: " << Configuration::windows.size() << "\n";
        LOG << " * Shading: " << Configuration::shades.size() << "\n";
...
const std::string Configuration::stringToLower(const std::string str1)
    {
...
bool Configuration::attributeNameIs(const rapidxml::xml_attribute<> *attribute,
                                    const char * str1)
    {
    return strComp(stringToLower(attribute->name()).c_str(), str1);
    }
